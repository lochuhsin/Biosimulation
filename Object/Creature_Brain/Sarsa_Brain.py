...
class Brain_Q_table_move():
    ...
    def __init__(self,action_dic):             
        ### only unpack move options 
        ### create q-table
        
        self.move_action = action_dic['move_option']  
        self.q_table_column = action_dic['move_option'] + ['state']
        ...
        ### uses step count to represent states
        self.origin_state = 0
        self.new_state = 0
        self.reward = 0
        
        ### i said i wanna use vis map to represent but .... there are some bugs , just wait a few moments
        self.count  = 0
    ...
    def get_action_move(self):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            print('state action for valuse : ',state_action)
            print('action which has been choosen : ',action)
        else:
    ...
    def __learn(self):
        ...
        print('q_table after learn: ',self.q_table.iloc[:,:-1])
